...
export class AuthService {
  ...
  async register(dto: AuthDto) {
    ...
    let oldUser = await this.userService.getByEmail(dto.email);
    !oldUser ? oldUser = await this.userService.getByEmail(dto.email) : ''
  ...
  async getNewTokens(refreshTokens: string) {
    const result = await this.jwt.verify(refreshTokens);
