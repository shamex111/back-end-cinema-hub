...
import { returnPaymentObject } from './objects/return-payment.object';
...
export class PaymentService {
  ...
  async checkout(dto: PaymentDto, userId: string) {
    ...
    if (user.isHasPremium)
      throw new ConflictException('У пользователя уже есть премиум-подписка');
  ...
  async delete(id: string) {
    try {
      const deleteReview = await this.prisma.payment.delete({
        where: { id },
      });
      return deleteReview;
    } catch (error) {
